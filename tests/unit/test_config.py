...
sample_invalid_config_file = os.path.join(current_dir, 'data', 'invalid_config.yaml')
sample_invalid_yml_file = os.path.join(current_dir, 'data', 'invalid_config_yaml.yaml')
mock_log = MagicMock()
...
def test_generate_config_file_success(mock_render_template, mock_isdir, mock_file_open):
...
"""
    ...
    mock_file_open.assert_called_once_with(os.path.join(path, "config.yml"), "w", encoding="utf-8")
...
@patch("os.path.isdir")
@patch("fits2db.config.config.render_template")
def test_generate_config_failure(mock_render_template, mock_isdir, mock_file_open):
